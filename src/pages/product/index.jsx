...
import HandInput from "../../components/product/hand/ProductInput";
import ProductList from "../../components/product/hand/ProductList";
...
const index = () => {
    ...
    const tabsByCategory = {
        ...
        hair: ['ALL','컨디셔너','오일&에센스','로션','스크럽', '세럼', '샴푸', '마스크/트리트먼트', 'Gift&Set'],
        body: ['ALL','바스&샤워젤','로션&크림','솝', '오일', '스크럽', '미스트', '마스크/트리트먼트', 'Gift&Set'],
        skin: ['ALL','수분&영양 크림','세럼&오일','클렌저', '토너/에센스', '미스트', '아이/넥 케어', '립 케어', 'Gift&Set'],
                    ...
                    <ul className="tabs">
                       {
                        currentTabs.map((tab, idx)=>(
                            <li key={tab} className={activeTab === tab ? "on" : ""} onClick={()=>setActiveTab(tab)}>{tab}</li>
                        ))
                       }
                    </ul>
                    <div className="sort">
