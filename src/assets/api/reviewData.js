...
const sampleContents = [
  ...
  const generateReview = (productId, idx) => ({
    reviewId: `${productId}-${idx}`,
  ...
  const allReviews = [];
  for (let productId = 1; productId <= 247; productId++) {
    const reviewCount = Math.floor(Math.random() * 16); // 0~15개
    const reviews = Array.from({ length: reviewCount }, (_, idx) => generateReview(productId, idx));
